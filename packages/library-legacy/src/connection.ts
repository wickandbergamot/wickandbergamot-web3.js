...
/**
 * Network Inflation
 * (see https://docs.wickandbergamot.org/implemented-proposals/ed_overview)
 */
...
/**
 * Leader schedule
 * (see https://docs.wickandbergamot.org/terminology#leader-schedule)
 */
 ...
 *
 * @deprecated Deprecated since Wickandbergamot v1.8.0.
 */
  ...
  if (process.env.BROWSER) {
    ...
    if (httpAgent == null) {
      if (process.env.NODE_ENV !== 'test') {
        const agentOptions = {
          // One second fewer than the Wickandbergamot RPC's keepalive timeout.
...
export class Connection {
  ...
  async getInflationRate(): Promise<InflationRate> {
    ...
    if ('error' in res) {
      throw new WickandbergamotJSONRPCError(res.error, 'failed to get inflation rate');
    }
    return res.result;
  }
  ...
  async getFeeForMessage(
    ...
    if ('error' in res) {
      throw new WickandbergamotJSONRPCError(res.error, 'failed to get fee for message');
    }
    if (res.result === null) {
  ...
  > {
    ...
    try {
      ...
      throw new WickandbergamotJSONRPCError(e as JSONRPCError, 'failed to get block');
    }
  }
